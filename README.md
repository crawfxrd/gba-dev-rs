...
- [Rust] and Cargo
- clang for compiling the non-Rust code
...
clang is used for compiling the non-Rust code. Assembly must be used to write
the ROM header and the master ISR. Any addition ARM functions would also have
to be written in assembly or C, as mixing Thumb and ARM states in Rust is
[unusably inefficient][instruction_set].

## Building

Cargo is [configured][cargo-config] to run with the custom ARM7TDMI target
`armv4t-none-eabi`.
...
```
cargo build --release
rust-objcopy -O binary target/armv4t-none-eabi/release/untitled target/untitled.gba
```
